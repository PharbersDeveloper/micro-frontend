<template>
    <div class="pagination-container">
        <div class="pagination">
            <span @click="changePage('pre')">
                <div id="chevron-left" :class="curPage === 1 ? 'chevron_left_unable cursor-not-allowed' : 'chevron_left cursor-pointer'"></div>
            </span>

            <div v-for="(page,index) in forEachArray" :key="index" @click="changePage('',page)" :class="curPage === page ? 'pagination-page pagination-active' : 'pagination-page'">{{page}}</div>
            
            <span @click="changePage('next')">
                <div id="chevron-right" :class="curPage === pageArray.length ? 'chevron_right_unable cursor-not-allowed' : 'chevron_right cursor-pointer'"></div>
            </span>
        </div>
    </div>
</template>
...
export default {
    ...
    methods: {
        ...
        changePage(type, page) {
            if (type === 'next') {
                this.curPage = Math.min(this.curPage + 1, this.pageArray.length)
