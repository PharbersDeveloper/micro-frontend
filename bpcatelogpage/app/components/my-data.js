		...
		try {
			...
			await s3Client
				...
				.on( "httpUploadProgress", function ( progress ) {
					that.set( "uploadLoadedSize", progress.loaded )
				} )
				.promise()
			/**
			...
			// that.set( "showProgress", false ) // 关闭上传进度条
			// that.set( "uploadFileStatus", fileObj )

			//上传成功提示
			// that.toast.success( "", "上传成功", that.toastOptions )
			// that.set( "uploadTextStatus", "上传成功" )
			// that.set( "uploadText", "在“我的数据”中查看结果" )
			// that.set( "uploadToastBorder", "upload-toast-border-green upload-toast" )
			// that.set( "uploadToastImg", "check_circle-24px" )
		} catch ( e ) {
			...
			that.set( "uploadTextStatus", "上传失败" )
			that.set( "uploadText", "" )
			that.set( "uploadToastBorder", "upload-toast-border-red upload-toast" )
			that.set( "uploadToastImg", "cancel-24px" )
		}
